...
Quaternion Quaternion::multiply(const Quaternion& another) const{
...
Quaternion Quaternion::fromAnglerVelocity(float avx,float avy,float avz,float time) const{
...
Vector Quaternion::toVector(void) const{//vector型からもどすとき専用!
	return Vector(x,y,z);
}
